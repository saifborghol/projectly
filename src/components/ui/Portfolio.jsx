...
const Portfolio = () => {
  ...
  const getCardStyle = (index) => {
    const stackThreshold = 0.1 + index * 0.2;
    const stackProgress = Math.max(
      0,
      Math.min(1, (scrollY - stackThreshold) / 0.4)
    );

    if (stackProgress <= 0) {
      return {
        transform: "translateY(0px) scale(1) rotateX(0deg) translateZ(0px)",
    ...
    const translateY = stackProgress * 60;
    const translateZ = -stackProgress * 100;
    const scale = 1 - stackProgress * 0.1;
    const rotateX = stackProgress * 8;
    const newZIndex =
